...
def run_derivation(generate_eqs):
    y, sigma_y = symbols("y, sigma_y")
    state_vector = Matrix([y])
    f = Matrix([y])
    variance_vector = Matrix([sigma_y])
    F = f.jacobian(state_vector)
    Q = F * diag(*variance_vector) * F.T

    obs_baro_var, obs_var_gps = symbols("R_BARO, R_GPS")
    h = Matrix([y, y])
    H = h.jacobian(state_vector)
    R = diag(obs_baro_var, obs_var_gps)
    ...
    if generate_eqs:
        ...
        handles["f"] = lambdify([*state_vector], f)
        handles["F"] = lambdify([*state_vector], F)
        handles["Q"] = lambdify([*state_vector, *variance_vector], Q)
        handles["h"] = lambdify([*state_vector], h)
        handles["H"] = lambdify([*state_vector], H)
        handles["R"] = lambdify([obs_baro_var, obs_var_gps], R)
...
# def _create_symmetric_cov_matrix(n):
#     P = Matrix(n, n, _create_cov_matrix)
...
# def _generate_observation_equations(P, state, H, R):
#     n = H.shape[0]
...
#     gyro_x_bias, gyro_y_bias, gyro_z_bias = symbols(
#         "gyro_x_bias, gyro_y_bias, gyro_z_bias"
#     )
#     gyro_bias = Matrix([gyro_x_bias, gyro_y_bias, gyro_z_bias])

#     accel_x_bias, accel_y_bias, accel_z_bias = symbols(
...
#     sigma_accel_x, sigma_accel_y, sigma_accel_z = symbols(
#         "sigma_accel_x, sigma_accel_y, sigma_accel_z"
#     )
#     sigma_gyro_x, sigma_gyro_y, sigma_gyro_z = symbols(
#         "sigma_gyro_x, sigma_gyro_y, sigma_gyro_z"
#     )
...
#         q,
#         Matrix(
#             [1, gyro_truth[0] * dt / 2, gyro_truth[1] * dt / 2, gyro_truth[2] * dt / 2]
#         ),
#     )
...
#     h_mag, H_mag, R_mag, R_mag_var = generate_mag_observation(
#         state_vector, rot_to_body, mag, mag_bias
#     )

#     if generate_eqs:
#         print("Generating Covariance Prediction...")

#         P = _create_symmetric_cov_matrix(state_vector.shape[0])

#         code_gen.write_cov_matrix(
#             "covariance_prediction", generate_cov_prediction(P, F, Q)
#         )
...
#         code_gen.write_obs_eqs(
#             "baro_fusion",
#             _generate_observation_equations(P, state_vector, H_baro, R_baro),
#         )
#         code_gen.write_obs_eqs(
#             "mag_fusion", _generate_observation_equations(P, state_vector, H_mag, R_mag)
#         )
...
#             ),
#         )
#         functions_handles["R"] = lambdify(
#             [R_gps_var, R_baro_var, R_mag_var], Matrix.diag(R_gps, R_baro, R_mag)
#         )
