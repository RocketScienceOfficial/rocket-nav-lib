...
import numpy as np
from . import ekf, analysis, derivation, data_provider
...
measurements, flags, controls = data_provider.generate_scenario_free_fall_parachute(
    1000, config['dt'], config['g'], 1000, control_variances | meas_variances)

n_m = measurements.shape[1]
filt = ekf.ExtendedKalmanFilter(init_params['x0'], init_params['P0'], n_m, config['g'], config['dt'], list(control_variances.values()))
